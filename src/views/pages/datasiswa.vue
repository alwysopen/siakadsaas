    ...
    <b-row>
      <b-col xs="12">
        <b-card>
          ...
          <b-modal title="Ubah Data Siswa" v-model="myModal1" @ok="ActionUbah">
            <div class="form-group">
              <label>Nama Siswa</label>
              <input class="form-control" type="text" v-model="namasiswa">
            </div>
            <div class="form-group">
              <label>Alamat</label>
          ...
          <b-modal title="Input Data Siswa" v-model="myModal" @ok="test">
            <div class="form-group">
              <label>Nama Siswa</label>
              <input class="form-control" type="text" v-model="namasiswa">
            </div>
         ...
         <table class="table bordered-stripped">
             ...
             <tbody>
                 <tr v-for="item in datasiswa">
                     <td>{{item.nama}}</td>
                     <td>{{item.alamat}}</td>
                     <td>{{item.kela.namakelas}}</td>
                     <td>{{item.jeniskelamin}}</td>
                     <td>{{item.email}}</td>
                     <td>{{item.nomortelpon}}</td>
                     <td><img :src="apiurl + '/'+ item.foto" width="100" height="100"></td>
                     <td> 
...
<script>
import siswaService from '../../services/datasiswa'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      var vm = this
      const result = await siswaService.list()
      vm.datasiswa = result
    },
    async loadDatakelas () {
      var vm = this
    ...
    async test () {
      ...
      formData.append('foto', 'siswafoto/' + this.file.name)
    ...
    async ActionUbah () {
      ...
      if (this.file === '') {
        ...
        let id = vm.id
        await siswaService.update(data, id)
        this.loadData()
      } else {
        let formData = new FormData()
